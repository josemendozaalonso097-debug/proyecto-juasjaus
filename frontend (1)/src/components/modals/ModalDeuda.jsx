...
export default function ModalDeuda({ isOpen, onClose, cantidad, onPayClick }) {
  if (!isOpen) return null;

  const adeudo = cantidad * 3000;
    ...
    <div id="modalDeuda" className="fixed inset-0 z-[1005] flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="relative w-full max-w-sm bg-white dark:bg-slate-900 rounded-2xl shadow-2xl overflow-hidden border border-red-500/30 mx-4 max-h-[90vh] flex flex-col">
        ...
        <div className="bg-red-50 dark:bg-red-900/20 px-6 py-6 border-b border-red-100 dark:border-red-900/50 flex flex-col items-center text-center relative z-10">
          ...
          <h2 className="text-xl font-black text-slate-800 dark:text-white leading-tight mb-1">Aviso Importante</h2>
        ...
        <div className="p-6 relative z-10 bg-white dark:bg-slate-900">
          <div className="text-center mb-6">
            <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
              Hemos detectado que tienes <strong id="modal-deuda-cantidad" className="text-slate-800 dark:text-slate-200">{cantidad}</strong> pago(s) pendiente(s) de colegiatura.
            </p>
          </div>
          ...
          <div className="bg-slate-50 dark:bg-slate-800 rounded-xl p-4 border border-slate-100 dark:border-slate-700 flex flex-col items-center justify-center mb-6">
            ...
            <span className="text-2xl font-black text-red-600 dark:text-red-500" id="modal-deuda-monto">${adeudo.toLocaleString('en-US')}.00 MXN</span>
          </div>
          ...
          <div className="flex flex-col gap-3">
            <button 
              ...
              className="w-full py-3 bg-gradient-to-r from-red-600 to-primary text-white font-bold rounded-xl shadow-lg shadow-red-500/20 hover:shadow-red-500/40 transition-all active:scale-95 flex items-center justify-center gap-2"
            >
            ...
            <button 
              onClick={onClose} 
              className="w-full py-3 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-bold rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-all active:scale-95"
            >
              Cerrar aviso
            </button>
          </div>
