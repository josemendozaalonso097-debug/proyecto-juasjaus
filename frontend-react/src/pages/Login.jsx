...
import {
    ...
    loginWithGoogleToken, checkSessionToken, checkBackendHealth,
        ...
        try {
            ...
            if (response.ok) {
                ...
                const userProfile = {
                    ...
                    nombre: userParams.nombre, rol: userParams.rol,
            ...
            <div className="relative w-full max-w-md bg-white dark:bg-slate-900 rounded-xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-700">
                <div className="bg-gradient-to-r from-primary to-red-700 px-6 py-5 text-center">
                    <div className="w-12 h-12 bg-white/20 rounded-full flex items-center justify-center mx-auto mb-3">
                        <span className="material-symbols-outlined text-white" style={{ fontVariationSettings: "'FILL' 1" }}>lock_open</span>
                    </div>
                ...
                <div className="p-6">
                    ...
                    <div className="flex flex-col gap-4">
                        <button type="button" onClick={handleVerify} disabled={!isComplete || verifying}
                        ...
                        <div className="text-center">
                            <div className="text-sm font-medium text-slate-600 dark:text-slate-400 mb-2">
                                El código expira en: <span className="font-bold text-primary">
...
function ForgotModal({ isOpen, onClose, onGoToSignup }) {
    ...
    const handleSend = async () => {
        if (!email.trim()) { showToast('Por favor ingresa tu email', 'warning'); return; }
        setSending(true);
        try {
    ...
    return (
        <div className="fixed inset-0 w-full h-full bg-black/50 z-[9999] flex justify-center items-center backdrop-blur-sm px-4"
            onClick={e => { if (e.target === e.currentTarget) { onClose(); setEmail(''); } }}>
            <div className="relative w-full max-w-md bg-white dark:bg-slate-900 rounded-xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-700">
                <div className="bg-gradient-to-r from-primary to-orange-600 px-6 py-5 text-center">
                    <div className="w-12 h-12 bg-white/20 rounded-full flex items-center justify-center mx-auto mb-3">
                        <span className="material-symbols-outlined text-white" style={{ fontVariationSettings: "'FILL' 1" }}>password</span>
                    </div>
                ...
                <div className="p-6">
                    ...
                    <p className="text-center text-sm mt-3 text-slate-600 dark:text-slate-400">
                        ...
                        <a href="#" onClick={e => { e.preventDefault(); onClose(); onGoToSignup(); }}
                            className="text-primary font-bold hover:underline">Regístrate ahora</a>
                    </p>
                </div>
                <button type="button" onClick={() => { onClose(); setEmail(''); }}
...
export default function Login() {
    ...
    useEffect(() => {
        ...
        if (token) {
            ...
            checkSessionToken(token).then(res => {
                if (res.ok) navigate('/principal');
                else { localStorage.removeItem('access_token'); localStorage.removeItem('user'); }
            }).catch(() => { localStorage.removeItem('access_token'); localStorage.removeItem('user'); });
        }
        checkBackendHealth().then(r => r.ok && r.json().then(d => console.log('✅ Backend:', d))).catch(() => {});
    }, [navigate]);
    ...
    const handleLogin = async (e) => {
        e.preventDefault();
        if (!loginEmail || !loginPassword) { showToast('Por favor llena todos los campos', 'warning'); return; }
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data.user.id) {
                    ...
                    if (!prev) localStorage.setItem(`perfil_${data.user.id}`, JSON.stringify(data.user));
                }
    ...
    const handleRegister = async (e) => {
        ...
        if (!regNombre || !regEmail || !regPassword || !regRol) {
            showToast('Por favor llena todos los campos, incluyendo tu rol', 'warning'); return;
        }
        if (regRol === 'estudiante' && !regSemestre) {
            showToast('Por favor selecciona tu semestre', 'warning'); return;
        }
        if (regPassword.length < 6) {
            showToast('La contraseña debe tener al menos 6 caracteres', 'warning'); return;
        }
        setRegLoading(true);
        try {
            const userData = {
                nombre: regNombre, email: regEmail, password: regPassword,
                rol: regRol, semestre: regSemestre ? parseInt(regSemestre) : null
            };
            ...
            if (response.ok) {
                ...
                setOtpUserParams({ nombre: regNombre, rol: regRol, semestre: regRol === 'estudiante' ? regSemestre : null });
                setShowOtp(true);
            } else {
    ...
    return (
        <div className="login-page bg-background-light dark:bg-background-dark min-h-screen md:flex md:items-center md:justify-center p-0 md:p-4">
            <div className={`container-box w-full md:max-w-[1200px] md:min-h-[700px] bg-background-light dark:bg-background-dark md:bg-white md:dark:bg-slate-900 md:shadow-2xl mx-auto rounded-none md:rounded-xl ${isActive ? 'active' : ''}`}>
                ...
                <div className="form-container sign-up sign-up-container bg-background-light dark:bg-background-dark md:bg-white md:dark:bg-slate-900 px-0 py-0 md:px-16 md:py-8 flex flex-col justify-start md:justify-center">
                    ...
                    <header className="md:hidden flex items-center justify-between px-4 py-4 bg-background-light dark:bg-background-dark sticky top-0 z-50">
                        <button aria-label="Volver" onClick={() => setIsActive(false)} className="text-primary cursor-pointer">
                            <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M10.5 19.5L3 12m0 0l7.5-7.5M3 12h18" strokeLinecap="round" strokeLinejoin="round" /></svg>
                        </button>
                    ...
                    <section className="md:hidden relative w-full h-64 overflow-hidden mb-8">
                        ...
                        <div className="absolute bottom-6 left-0 right-0 px-6">
                            <h2 className="text-white text-3xl font-black uppercase tracking-wide leading-tight">UN MOTIVO DE ORGULLO</h2>
                        </div>
                    </section>
                            ...
                            <form className="space-y-4" onSubmit={handleRegister}>
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input type="email" value={regEmail} onChange={e => setRegEmail(e.target.value)}
                                            style={getInputStyle('email', regEmail)}
                                            className="block w-full pl-10 pr-3 py-3 border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all"
                                            placeholder="example@cbtis258.edu.mx" required />
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input type={showRegPwd ? 'text' : 'password'} value={regPassword} onChange={e => setRegPassword(e.target.value)}
                                            style={getInputStyle('password', regPassword)}
                                        ...
                                        <i className={`fa-solid ${showRegPwd ? 'fa-eye-slash' : 'fa-eye'} absolute right-4 top-[14px] text-slate-400 cursor-pointer hover:text-primary`}
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <select value={regRol} onChange={e => { setRegRol(e.target.value); if (e.target.value !== 'estudiante') setRegSemestre(''); }}
                                ...
                                {regRol === 'estudiante' && (
                                    <div className="transition-all duration-300">
                                        ...
                                        <div className="relative">
                                            ...
                                            <select value={regSemestre} onChange={e => setRegSemestre(e.target.value)}
                                                className="block w-full pl-10 pr-10 py-3 border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all appearance-none cursor-pointer">
                                                <option disabled value="">Selecciona tu semestre</option>
                                                {[1,2,3,4,5,6].map(s => (
                                                    <option key={s} value={s}>{['1er','2do','3er','4to','5to','6to'][s-1]} Semestre</option>
                                                ))}
                                            </select>
                                            <div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                                                <span className="material-symbols-outlined text-slate-400">expand_more</span>
                                ...
                                <div className="flex flex-col gap-2">
                                    ...
                                    <div className="relative flex items-center">
                                        ...
                                        <input type="email" value={loginEmail} onChange={e => setLoginEmail(e.target.value)}
                                            style={getInputStyle('email', loginEmail)}
                                ...
                                <div className="flex flex-col gap-2">
                                    ...
                                    <div className="relative flex items-center">
                                        ...
                                        <input type={showLoginPwd ? 'text' : 'password'} value={loginPassword} onChange={e => setLoginPassword(e.target.value)}
                                            style={getInputStyle('password', loginPassword)}
                                        ...
                                        <i className={`fa-solid ${showLoginPwd ? 'fa-eye-slash' : 'fa-eye'} absolute right-4 text-slate-400 cursor-pointer hover:text-primary`}
                                            onClick={() => setShowLoginPwd(!showLoginPwd)} />
                                    </div>
                                    <div className="text-right mt-1">
                                        <a className="text-primary text-sm font-medium hover:underline cursor-pointer"
                                            onClick={() => setShowForgot(true)}>¿Olvidaste tu contraseña?</a>
                                    </div>
                                </div>
                    ...
                    <div className="overlay bg-primary">
                        <div className="absolute inset-0 opacity-10">
                            <svg height="100%" width="100%" xmlns="http://www.w3.org/2000/svg">
                                <defs><pattern height="40" id="grid1" patternUnits="userSpaceOnUse" width="40"><path d="M 40 0 L 0 0 0 40" fill="none" stroke="white" strokeWidth="1" /></pattern></defs>
                                <rect fill="url(#grid1)" height="100%" width="100%" />
                            </svg>
                        </div>
            ...
            {/* Modals */}
            <ForgotModal isOpen={showForgot} onClose={() => setShowForgot(false)} onGoToSignup={() => setIsActive(true)} />
            <OtpModal isOpen={showOtp} onClose={() => setShowOtp(false)} email={otpEmail} userParams={otpUserParams} navigate={navigate} />
        </div>
    );
}
